...
{
    public class SHA512_224 : IDisposable
    {
        private SHA512_224() { }

        public const int DigestSize = 224;
        ...
        private uint[] MessageDigest(uint[][] blocks)
        {
            ...
            return H0.Concat(H1).Concat(H2).Concat(H3).Concat(H4).Concat(H5).Concat(H6).Concat(H7).Take(DigestSize).ToArray();
        }
